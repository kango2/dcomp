...
rna <- read_delim("~/Downloads/dcomp-main/data/OAN/mdo.normalised.counts.txt", delim = "\t")
tissues <- c("Fib")
prot <- list()
for (i in 1:length(tissues)){
    x <- read_delim(paste("~/Downloads/dcomp-main/data/Prot_ratios/MDO", tissues[i],".txt",sep=""), delim = "\t")
...
colnames(prot)[8] <- c("pFib")

colnames(rna)[1] <- c("geneid")

medians <- bind_rows(rna %>% mutate(rFib = dfm1/dff1) %>%
select(chr,start=gstart,ratio=rFib) %>% mutate(type = "RNA"),
select(prot, chr, start=pstart, ratio=pFib) %>% mutate(type = "Protein")) %>% filter(chr %in% c("chr7","chrX")) %>%
...
MDOFibChrplot1 <- bind_rows(rna %>% mutate(rFib = dfm1/dff1) %>%
select(chr,start=gstart,ratio=rFib) %>% mutate(type = "RNA"),
select(prot, chr, start=pstart, ratio=pFib) %>% mutate(type = "Protein")) %>% filter(chr %in% c("chr7","chrX")) %>%
mutate(ratio = case_when(ratio > 4 ~ 4, TRUE ~ ratio)) %>%
...
pdf("~/Downloads/MDOFibChrplot1.pdf", height=2.5, width=11)
MDOFibChrplot1
dev.off()
...
##for boxplots
autosomes <- paste("chr", c(1:8),sep="")

MDOFibplot1 <- bind_rows(rna %>% mutate(rFib = dfm1/dff1) %>% 
            select(chr,geneid, start=gstart,ratio=rFib) %>% mutate(type = "RNA"), 
          select(prot, geneid, chr, start=pstart, ratio=pFib) %>% mutate(type = "Protein")) %>% filter(chr %in% c(autosomes, "chrX")) %>% 
...
theme_bw() + theme(text = element_text(size=20)) + coord_cartesian(ylim = c(-1, 1)) + xlim(c("autosome","chrX")) +
labs(fill="")
pdf("~/Downloads/MDOFibplot1.pdf", height=4, width=12)
MDOFibplot1
dev.off()
...
#calculate p values
ratios <- bind_rows(rna %>% mutate(rFib = dfm1/dff1) %>%
                      select(chr,geneid,start=gstart,ratio=rFib) %>% mutate(type = "RNA"),
                    select(prot, geneid, chr, start = pstart, ratio = pFib) %>% mutate(type = "Protein")) %>% filter(chr %in% c(autosomes,"chrX")) %>%
  mutate(ctype = case_when(chr %in% autosomes ~ "autosome", TRUE ~ chr)) %>%
  filter(!is.na(ratio)) %>%
...
library("writexl")
write_xlsx(p_adjust_values_withautosomes, "~/Downloads/MDO_Fib_p_values_with_autosomes.xlsx")
